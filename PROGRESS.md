...
마지막 업데이트: 2026-05-19 (Stage 16-3: 호가 단위 자동 보정 + 잔고/예수금 표시 — 토스 풀스택 완료)
...
### Phase 1 Stage 16-3: 호가 단위 자동 보정 + 잔고 표시 ✨ NEW
Stage 16-2의 OrderEntryScreen에 한투 거부 방지(호가 단위)와 사전 확인(예수금/보유)을 추가해 토스 수준 UX 마무리.

**`domain/order/PriceTickRules.kt` 신설**:
- `tickFor(market, price)` — KOSPI/KOSDAQ 가격대별 1/5/10/50/100/500/1000원, US는 단순 $0.01.
- `snap(market, price)` — 가장 가까운 valid tick으로 보정 (한투 APBK0918/0919 사전 방지).
- `isValid(market, price)` — 검증 함수 (현재 UI는 안내만, 자동 보정).

**OrderEntryViewModel 통합**:
- `AccountService` 주입 — init 시 Market에 따라 fetchKrBalance / fetchUsBalance 호출.
- AccountSummary.cashDeposit → `cashDeposit` 필드.
- Holdings 중 argTicker 매칭 항목 → `holdingQty` / `holdingAvgPrice` 필드.
- `submit()`에서 LIMIT 가격은 `snappedPrice`(자동 보정값)로 한투 송신.

**OrderEntryUiState 추가 derived properties**:
- `snappedPrice` — 보정된 가격.
- `priceNeedsSnap` — 입력값이 보정값과 다르면 true (UI 안내 표시).
- `tickSize` — 현재가 기준 호가 단위.
- `insufficientWarning` — 매수 시 예상금액 > 예수금이면 "예수금 초과", 매도 시 수량 > 보유 수량이면 "보유 수량 초과 (보유 N주)". null이면 정보 부족 또는 충분.

**UI 보강**:
- 현재가 카드에 "호가 단위 N원" 부가 표시.
- 신규 잔고/보유 카드 — 매수면 "주문 가능 예수금", 매도면 "보유 N주 · 평균 가격".
- 가격 입력 필드 아래에 보정 안내: `→ 호가 단위 보정: ${snappedPrice}원에 송신` (LIMIT + 입력값이 보정값과 다를 때만).
- 예상 금액 아래에 `⚠️ 예수금 초과 / 보유 수량 초과` 경고 텍스트 (errorColor).

**Stage 16-3 OOS (다음 보강 후보)**:
- 한투 hashkey 헤더 발급 — 운영에서 hashkey 강제 응답 받으면 KisAuthService.ensureHashKey 추가.
- 다단계 호가 단위 (저가 종목 미세 tick) — 현재는 단순 7단계.
- 미체결 polling을 ApplicationScope에서 launch — 현재는 viewModelScope, 화면 떠나면 polling 끊김 (Stage 9 TradeImportService가 24h 자동 import로 보강하긴 함).
- 일일 주문 한도(설정에 누적 한도) — 사용자 결정 "미선택"으로 영구 OOS.

✅ `./gradlew.bat assembleDebug` BUILD SUCCESSFUL (17s)
...
**바로 다음**: 실기기 검증.
- `./gradlew.bat installDebug` 후 Z Fold 7에서 종목 상세 진입 → 차트 토글(분/일/주/월/년/캔들/라인) → 매수·매도 BottomBar → OrderEntryScreen → BiometricPrompt → 한투 송신 → OrdersScreen에서 상태 추적까지 전 흐름 확인.
- 한투 H0STASP0 호가 응답 필드 인덱스 (KOSPI_ASKING_FIELDS_PER_TICK=59, ANTC_CNPR=56) 운영값과 매칭되는지 검증. 불일치면 KisWebSocketStream.parseKospiAskingTick 상수 조정.
- 한투 주문 응답에서 hashkey 강제 여부 확인. 강제면 KisAuthService.ensureHashKey + KisTradingApi의 hashKey 헤더 채우는 로직 추가.
